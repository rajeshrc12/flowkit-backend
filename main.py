    ...
    try:
        chat = client.chats.create(
            ...
            history=[
                genai.types.Content(
                    role=item["role"],
                    parts=[genai.types.Part(text=part) for part in item["parts"]]
                )
                for item in state["chat_message"]
            ]
        )
        # Send current message
        state["chat_message"].append({"role": "user", "parts": [state["message"]]})
        response = chat.send_message(state["message"])
        state["chat_message"].append({"role": "model", "parts": [response.text]})
        return state
